...
@implementation YSKJ_SerLbLayout
...
-(UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSArray* titles = [self.delegate serchLabelLayoutTitlesForLabel];
    ...
    CGFloat width = titleSize.width + 50;
    CGFloat height = titleSize.height + 24;
    
    NSInteger currentRow = 0;
    for (NSInteger i = 0 ; i < self.numberOfRow; i ++ ) {
        
        if (titleSize.width + [self.maxXArray[i] floatValue] + panding + 50<= self.collectionView.bounds.size.width - panding) {
            currentRow = i ;
            break;
        }
