...
-(id)initWithFrame:(CGRect)frame
{
    if (self == [super initWithFrame:frame]) {
        ...
        UIButton *deleteBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.frame.size.width - 44, 0, 44, 44)];
        [deleteBtn setImage:[UIImage imageNamed:@"Delete1"] forState:UIControlStateNormal];
        deleteBtn.imageEdgeInsets = UIEdgeInsetsMake(0, 22, 22, 0);
        _delBtn = deleteBtn;
...
-(void)deleteAction:(UIButton*)sender
{
    ...
    if (self.delegate != nil && [self.delegate respondsToSelector:@selector(deleteRow:)]) {
        [self.delegate deleteRow:indexPath.row];
    }
}
